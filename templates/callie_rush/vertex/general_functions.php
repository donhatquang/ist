...
if(strrpos($version_number,"1.5.") > 1) {
	...
	if(!empty($s5_hide_component_items)) {  
		...
		if ($s5_hide_component_array_count == 1) {
			...
			if ($s5_hide_component_items == JSite::getMenu()->getActive()->id) {
				$s5_show_component = "no";
			}
		}
	...
	if(!empty($s5_hide_component_items)) {  
		...
		if ($s5_hide_component_array_count > 0) {
			for ($s5_hide_component_array_holder = 0; $s5_hide_component_array_holder < $s5_hide_component_array_count; $s5_hide_component_array_holder++) {
				
				//var_dump($s5_hide_component_items[$s5_hide_component_array_holder]);
				//exit();
				

				if (isset(JSite::getMenu()->getActive()->id) && $s5_hide_component_items[$s5_hide_component_array_holder] == JSite::getMenu()->getActive()->id) {
					$s5_show_component = "no";
				}
