...
/*
 * Vertex makeVertexSelects
 */

var autoSelectVal = 'automatic';
var widths = [];
widths[0] = 100;
widths[1] = 50;
widths[2] = 33.33;
...
function updateInput(a,b){var c=jQuery('#'+a);var d=jQuery(c).val();d=d.split(',');var e=[];e[0]=(b[0]?b[0]:d[0]);e[1]=(b[1]?b[1]:d[1]);e[2]=(b[2]?b[2]:d[2]);e[3]=(b[3]?b[3]:d[3]);e[4]=(b[4]?b[4]:d[4]);e[5]=(b[5]?b[5]:d[5]);e=e.join(',');c.val(e)}

function refreshInput(a,b){var c=jQuery('#'+a);var d=[];d[0]=b[0];d[1]=b[1];d[2]=b[2];d[3]=b[3];d[4]=b[4];d[5]=b[5];d=d.join(',');c.val(d)}

function sliderCalc(a){var b=[];b[0]=Number(a[0]);b[1]=Number(a[0])+Number(a[1]);b[2]=Number(a[0])+Number(a[1])+Number(a[2]);b[3]=Number(a[0])+Number(a[1])+Number(a[2])+Number(a[3]);b[4]=Number(a[0])+Number(a[1])+Number(a[2])+Number(a[3])+Number(a[4]);b[5]=Number(a[0])+Number(a[1])+Number(a[2])+Number(a[3])+Number(a[4])+Number(a[5]);return b}

function makeError(a,b){var c=Number(b[0])+Number(b[1])+Number(b[2])+Number(b[3])+Number(b[4])+Number(b[5]);var d='Im sorry but '+b[0]+' + '+b[1]+' + '+b[2]+' + '+b[3]+' + '+b[4]+' + '+b[5]+' only equals '+c+'% it must equal 100%';return d}

/*
 * Vertex makeVertexDropdowns
 */

function makeVertexDropdowns(m, n, o, p, q) {
    var i = 0;
    var r = 0;
        ...
        _create: function () {
            ...
            var f = this.input = jQuery('<input>').attr('size', 25).insertAfter(select).val(value).autocomplete({
                ...
                open: function (a, b) {
                    ...
                    jQuery(this).autocomplete("resizemenu", c)
                },
                change: function (a, b) {
                    if (!b.item) {
    ...
    jQuery('.panelform .adminformlist').find('select').each(function () {
        var a = jQuery(this);
        var b = a.attr('id');
        if (!a.attr('multiple')) {
            if (b.length) {
                if (!jQuery.contains(b, 'calculation')) {
                    jQuery(a).combobox();
                    i++
                }
            }
    ...
    var i = 0;
    jQuery('.panelform .adminformlist').find('textarea').each(function () {
        var a = jQuery(this);
        var j = a.attr('id');
        a.hide();
        var k = [];
        ...
        var n = 0;
        value2 = m.split(',');
        jQuery.each(value2, function (a, b) {
            l[a] = b;
            if (l[a] == 0) {
                k[a] = 0
            } else {
        ...
        var o = jQuery('<div />').addClass('slider-val').css({
            width: '100%'
        }).text('Automatic').hide();
        var p = jQuery('<div />').addClass('slider-val').css({
            width: k[0] + '%'
        }).text(k[0] + '%').hide();
        var q = jQuery('<div />').addClass('slider-val').css({
            width: k[1] + '%'
        }).text(k[1] + '%').hide();
        var r = jQuery('<div />').addClass('slider-val').css({
            width: k[2] + '%'
        }).text(k[2] + '%').hide();
        var s = jQuery('<div />').addClass('slider-val').css({
            width: k[3] + '%'
        }).text(k[3] + '%').hide();
        var t = jQuery('<div />').addClass('slider-val').css({
            width: k[4] + '%'
        }).text(k[4] + '%').hide();
        var u = jQuery('<div />').addClass('slider-val').css({
            width: k[5] + '%'
        }).text(k[5] + '%').hide();
        var v = a.parent().parent('li').prev('li').children('.item-value').children('select').children('option:selected').val();
        var w = true;
        var x = [];
        if (v == autoSelectVal) {
            o.hide();
        ...
        if (v > 2) {
            w = false;
            x[2] = Number(l[0]) + Number(l[1]) + Number(l[2]);
            r.show()
        }
        if (v > 3) {
            w = false;
            x[3] = Number(l[0]) + Number(l[1]) + Number(l[2]) + Number(l[3]);
            s.show()
        }
        if (v > 4) {
            w = false;
            x[4] = Number(l[0]) + Number(l[1]) + Number(l[2]) + Number(l[3]) + Number(l[4]);
            t.show()
        }
        if (v > 5) {
            w = false;
            x[5] = Number(l[0]) + Number(l[1]) + Number(l[2]) + Number(l[3]) + Number(l[4]) + Number(l[5]);
            u.show()
        }
        var y = 'sliderset_' + i;
        k.join(',');
        if (v != autoSelectVal) {
            var z = jQuery('<div />').attr('id', y + '_slider1').slider({
                range: true,
                min: 0,
                max: 100,
                step: 1,
                ...
                slide: function (a, b) {
                    if (b.values[0]) {
                        l[0] = b.values[0];
                        p.text(Math.round(l[0]).toString() + '%').css({
                            width: l[0] + '%'
                        })
                    }
                    if (b.values[1]) {
                        l[1] = b.values[1] - b.values[0];
                        q.text(Math.round(l[1]).toString() + '%').css({
                            width: l[1] + '%'
                        })
                    }
                    if (b.values[2]) {
                        l[2] = b.values[2] - b.values[1];
                        r.text(Math.round(l[2]).toString() + '%').css({
                            width: l[2] + '%'
                        })
                    }
                    if (b.values[3]) {
                        l[3] = b.values[3] - b.values[2];
                        s.text(Math.round(l[3]).toString() + '%').css({
                            width: l[3] + '%'
                        })
                    }
                    if (b.values[4]) {
                        l[4] = b.values[4] - b.values[3];
                        t.text(Math.round(l[4]).toString() + '%').css({
                            width: l[4] + '%'
                        })
                    }
                    if (b.values[5]) {
                        l[5] = b.values[5] - b.values[4];
                        u.text(Math.round(l[5]).toString() + '%').css({
                            width: l[5] + '%'
                        })
                    }
                    updateInput(j, l)
        ...
        if (v == autoSelectVal) {
            var z = jQuery('<div />').attr('id', y + '_slider' + i).hide()
        }
        var A = jQuery('<div />').attr('id', y + '_error').addClass('slider-bar-error').css({
            width: '100%',
            height: 20
        }).hide();
        var B = jQuery('<div />').attr('id', y + '_top').addClass('slider-preview-box').append(A).append(o).append(p).append(q).append(r).append(s).append(t).append(u);
        
        if (a.filter('textarea[id$="manual_widths"]')) {
            if (j.length) {
                var C = a.parent().parent('li').prev('li').children('.item-value').children('select');
                C.sizeslider({
                    width: 500,
                    curval: 'automatic',
                    selected: function (c, d) {
                        ...
                        u.hide();
                        var f = [];
                        jQuery(z).slider("destroy").remove();
                        var g = d.item.value;
                        if (g == autoSelectVal) {
                            l[0] = 0;
                            l[1] = 0;
                        ...
                        refreshInput(j, l);
                        if (g != autoSelectVal) {
                            z = jQuery('<div />').attr('id', y + '_slider1').slider({
                                range: true,
                                min: 0,
                                max: 100,
                                step: 1,
                            ...
                            jQuery(E).append(z);
                            jQuery(z).find('a:last').hide();
                            jQuery(z).find('a:first').text('1').next('a').text('2').next('a').text('3').next('a').text('4').next('a').text('5');
                            jQuery(z).find('a').click(function () {
                                jQuery(A).fadeOut(200).empty()
                            })
                        }
                        jQuery(B).find('div').removeClass('last');
                        jQuery(B).find('div').filter(":visible").filter(':last').addClass('last')
                    }
                });
                jQuery(z).find('a:last').hide();
                var D = jQuery('<a />').attr({
                ...
                }).addClass('slider-advanced').append('<span />').bind('click', function () {
                    var d = jQuery('<input />').addClass('inline-input').attr({
                        size: 2
                    }).val(l[0]).css({
                        'margin-top': 5
                    });
                    var e = jQuery('<input />').addClass('inline-input').attr({
                        size: 2
                    }).val(l[1]).css({
                        'margin-top': 5
                    });
                    var f = jQuery('<input />').addClass('inline-input').attr({
                        size: 2
                    }).val(l[2]).css({
                        'margin-top': 5
                    });
                    var g = jQuery('<input />').addClass('inline-input').attr({
                        size: 2
                    }).val(l[3]).css({
                        'margin-top': 5
                    });
                    var h = jQuery('<input />').addClass('inline-input').attr({
                        size: 2
                    }).val(l[4]).css({
                        'margin-top': 5
                    });
                    var i = jQuery('<input />').addClass('inline-input').attr({
                        size: 2
                    }).val(l[5]).css({
                        'margin-top': 5
                    });
                    ...
                    d.bind('keyup', function () {
                        l[0] = jQuery(this).val();
                        var a = Number(l[0]) + Number(l[1]) + Number(l[2]) + Number(l[3]) + Number(l[4]) + Number(l[5]);
                        if (Math.round(a) != 100) {
                            var b = makeError('error', l);
                            jQuery(A).text(b).fadeIn(500);
                            jQuery('#debug').empty();
                            jQuery('#debug').append(b + '<br/>')
                        } else if (Math.round(a) == 100) {
                            jQuery(A).fadeOut(500).empty();
                            updateInput(j, l);
                            var c = sliderCalc(l);
                            p.text(Math.round(l[0]).toString() + '%').css({
                                width: l[0] + '%'
                            ...
                            jQuery(z).slider("option", "values", c);
                            jQuery('#debug').empty()
                        }
                    });
                ...
                var E = jQuery('<div />').attr('id', y).addClass('slider-bar').append(z);
                var F = jQuery('<div />').attr('id', y + '_base').addClass('slider-base').append(D).append(B).append(E).append(A);
                jQuery(this).after(F);
                jQuery(z).find('a:first').text('1').next('a').text('2').next('a').text('3').next('a').text('4').next('a').text('5');
                jQuery(z).find('a').click(function () {
                    jQuery(A).fadeOut(200).empty()
    ...
    jQuery('.panelform .adminformlist').find('input:text').each(function () {
        var f = jQuery(this);
        var g = f.attr('id');
        var h = jQuery.inArray(g, p);
        var j = new Array(2);
        j['width'] = 150;
        j['overflow'] = 'hidden';
        if (g == 'block_position') {
                    ...
                    source: function (d, e) {
                        jQuery.ajax({
                            ...
                            success: function (b, c) {
                                j['width'] = b.extra.width;
                                j['overflow'] = b.extra.overflow;
                                e(jQuery.map(b.positions, function (a) {
                                    return {
                                        label: a.label,
                                        value: a.value
                                    }
                    ...
                    source: function (d, e) {
                        jQuery.ajax({
                            ...
                            success: function (b, c) {
                                j['width'] = b.extra.width;
                                j['overflow'] = b.extra.overflow;
                                e(jQuery.map(b.colors, function (a) {
                                    return {
                                        label: a.label,
                                        value: a.value
                                    }
...
/*
 * Vertex makeVertexToggle
 */
function makeVertexToggle(){jQuery('.vertex-admin-wrap .adminformlist li').find('input:radio:first').each(function(){jQuery(this).parent('fieldset').children('.item-name').hide();jQuery(this).parent('fieldset').children('input').hide();var a=jQuery(this);a.hide();a.parent('label').css({'font-size':'0'});var b=a.attr('id');if(b.length){jQuery('input#'+b).vertexToggle({type:'radio',onClickOff:function(){jQuery(this).parent().parent('.adminformlist label:last').find('input').attr('checked','checked')}})}})}

/*
 * Vertex changeColorBox
 */
function changeColorBox(i, hex) {jQuery('.vertex-admin-wrap').find('.adminformlist').find('#colorSelector' + i + ' div').css('backgroundColor', '#' + hex);}

/*
 * Vertex makeVertexColorpicker
 */
function makeVertexColorpicker(){var i=0;jQuery('.vertex-admin-wrap .adminformlist').find('input:text').each(function(){var d=jQuery(this);if(d.attr('size')==6||d.attr('size')==7){var s=d.attr('size');var e=d.attr('id');if(e.length){var f=d.val();if(s==6){jQuery('input#'+e).parent('.item-value').append('<div class="color-holder" style="z-index:9;"><'+'div id="colorSelector'+i+'"><div class="colorp-box" style="background-color:#'+f+';"></div></div></div>')}else{jQuery('input#'+e).parent('.item-value').append('<div class="color-holder" style="z-index:9;"><'+'div id="colorSelector'+i+'"><div class="colorp-box" style="background-color:'+f+';"></div></div></div>')}jQuery('#colorSelector'+i).ColorPicker({color:'#000000',onShow:function(a){jQuery(a).fadeIn(500);return false},onSubmit:function(hsb,hex,rgb,el){jQuery(el).ColorPickerHide();return false},onHide:function(a){jQuery(a).fadeOut(500);return false},onChange:function(a,b,c){jQuery('input#'+e).attr('value','#'+b);jQuery('input#'+e).parent('.item-value').children('div').children('div').children('div.colorp-box').css('backgroundColor','#'+b);jQuery('html').find('.'+e).css('backgroundColor','#'+b)}});i++}}})}

/*
 * Vertex makeDiscriptionTooltips
 */
function makeDiscriptionTooltips(c){var i=0;jQuery('.vertex-admin-wrap .adminformlist li').each(function(){var a=jQuery(this);if(jQuery(a).find("label").attr('title')!=''){var b=jQuery('<img src="'+c+'" height="24" width="24" alt="Info" />').css({'float':'right','position':'relative','right':'5px','margin':'0 0 5px'});jQuery(b).mouseenter(function(){if(jQuery(a).find("span.vertex-tt").css('opacity')!=0){jQuery(a).find("span.vertex-tt").css('opacity','0')}jQuery(a).find("span.vertex-tt").css('opacity','1');jQuery(a).find("span.vertex-tt").stop().fadeIn(350)}).mouseleave(function(){jQuery(a).find("span.vertex-tt").stop().fadeOut(350);jQuery(a).find("span.vertex-tt").css('opacity','0')});jQuery(a).children('div.item-name').append(b);jQuery(a).find("label").attr('title','')}});jQuery('.vertex-admin-wrap .adminformlist').each(function(){var count = jQuery(this).children('li').size();var a=jQuery(this).children('li').filter(":last");a.find('.vertex-tt').css('bottom',0);a.prev().find('.vertex-tt').css('bottom',0);if(count > 3) {a.prev().prev().find('.vertex-tt').css('bottom',0)}if(count > 4) {a.prev().prev().prev().find('.vertex-tt').css('bottom',0)}})}

var vertex_box;
var idPrefix = 'vpane_';

(function(jQuery){jQuery(document).ready(function(){var f=[];var g=[];var h=jQuery('<div />').attr('id','dafader1').addClass('vertex-admin-wrap dafader-wrapper');var j=jQuery('<ul />').attr('id','tabnav').addClass('fader-tabs');var k=jQuery('<ul />').attr('id','vertex-admin-menu').addClass('vertex-admin-menu');var i=0;jQuery('#style-form').find('.width-60').addClass('width-50').removeClass('width-60');jQuery('#style-form').find('.width-40').addClass('width-50').removeClass('width-40').css({'min-height':600,'margin-top':18});var l=jQuery('#style-form').find('.fltrt').children('.pane-sliders');jQuery(l).find('.panel').each(function(){var a=jQuery(this).children('h3').text();var b=jQuery(this).children('div.pane-slider').html();f[i]=a.replace(':','');g[i]=b;i++});i=0;l.empty();jQuery(f).each(function(i){if(i>6){var c=jQuery('<a />').attr('href','#'+idPrefix+i).addClass('df-tab').append(''+this).bind("click",function(){var a=jQuery.cookie('DFCurPanel');var b=jQuery(this).attr('href');jQuery('.fader-tabs .fader-tab').removeClass('active');jQuery('#vertex-admin-menu .fader-tab').removeClass('active');jQuery(this).parent('li').addClass('active');if(a!=b){jQuery('.da-fader-panel').hide(500);jQuery(b).show(500);jQuery.cookie('DFCurPanel',b)}return false});var d=jQuery('<li />').addClass('fader-tab').append(c);jQuery(j).append(d)}else if(i>0&&i<7){var c=jQuery('<a />').attr('href','#'+idPrefix+i).addClass('df-tab').append(''+this).bind("click",function(){var a=jQuery.cookie('DFCurPanel');var b=jQuery(this).attr('href');jQuery('.fader-tabs .fader-tab').removeClass('active');jQuery('#vertex-admin-menu .fader-tab').removeClass('active');jQuery(this).parent('li').addClass('active');if(a!=b){jQuery('.da-fader-panel').hide(500);jQuery(b).show(500);jQuery.cookie('DFCurPanel',b)}return false});var d=jQuery('<li />').addClass('fader-tab').append(c);jQuery(k).append(d)}i++});h.append(k).append(j);for(i=0;i<g.length;i++){var a=jQuery('<div />').addClass('da-fader da-fader-panel').attr('id',idPrefix+i).append(''+g[i]);h.append(a)}l.after(h);jQuery('#style-form').find('.fltrt').children('.pane-sliders').remove();var m=jQuery('#style-form').find('.fltrt').find('label');jQuery('#style-form').find('.fltrt').find('label').addClass('item-name-i').wrap(jQuery('<div />').addClass('item-name'));jQuery('#style-form').find('.fltrt').find('.adminformlist li').children().each(function(){if(!jQuery(this).hasClass('item-name')){jQuery(this).wrap(jQuery('<div />').addClass('item-value'))}});jQuery('#style-form').find('.fltrt').find('label').each(function(){var a=jQuery(this).attr('title');if(a){var b=a.split('::',2);var c=jQuery('<span />').addClass('tt-title').text(b[0]);var d=jQuery('<span />').addClass('tt-text').text(b[1]);var e=jQuery('<span />').addClass('vertex-tt').append(c).append(d).hide();jQuery(this).parent('div').append(e)}})})}(jQuery));

var vertexAdminImgPath = '../templates/callie_rush/vertex/admin/df-images/';
var vertexAdminHelpImg = '../templates/callie_rush/vertex/admin/df-images/help.png';
...
jQuery(document).ready(function() {
    ...
    if(savedPanel) {
        jQuery('.fader-tabs .fader-tab').find('a[href="' + savedPanel +'"]').parent('li').addClass('active');
        if(savedPanel == '#vpane_1') {
            jQuery('#vpane_1').show(500);
        } else {
            jQuery(savedPanel).show(500);
        }
    } else {
        jQuery.cookie('DFCurPanel', '#vpane_1');
        jQuery('#vpane_1').show(500);
    }
    function vertexLog(message) {
        jQuery('<' + 'div/>').text(message).prependTo('#vertexLog');
        jQuery('#vertexLog').attr('scrollTop', 0);
    }
    jQuery('.item-value').children('label').addClass('font-0');
    jQuery('.font-0').css("font-size","0");
    jQuery('.fader-panel .item-name span').hide();
    var ajaxURL = 'vertex2/vertexAdmin/vertexJSON.php';
    var showDelay = 500;
    var minWordLength = 2;
    ...
    makeDiscriptionTooltips(vertexAdminHelpImg);
    jQuery('.vertex-admin-wrap input:text').addClass('ui-widget ui-widget-content ui-corner-all vertex-input');
    makeVertexDropdowns(ajaxURL, showDelay, minWordLength, idArray, vertexLog);
