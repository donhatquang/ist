...
<?php foreach ($this->fields['attributes'] as $field) { ?>
	...
	<tr id="idFILESIZEDIAGNOSE2" style="display: none;">
		<td><?php echo JText::sprintf('RSFP_COMP_FIELD_FILESIZEDIAGNOSE_MSG',
			...
			ini_get('post_max_size') >= ini_get('upload_max_filesize') ? '<span class="rsform_ok">'.ini_get('post_max_size').'</span>' : '<strong class="rsform_notok">'.ini_get('post_max_size').'</strong>',
			ini_get('memory_limit') >= ini_get('post_max_size') ? '<span class="rsform_ok">'.ini_get('memory_limit').'</span>' : '<strong class="rsform_notok">'.ini_get('memory_limit').'</strong>'
			) ?></td>
	</tr>
	<?php } ?>
<?php } ?>
