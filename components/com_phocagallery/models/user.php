...
class PhocagalleryModelUser extends JModel
{
	...
	function __construct() {
		...
		$limitstart_subcat 	= JRequest::getVar('limitstartsubcat', 0, '', 'int');
		...
		$limitstart_image 	= JRequest::getVar('limitstartimage', 0, '', 'int');
		...
		if ((int)$id > 0) {
			...
			if (count( $cid )) {
				...
				if (count( $cid )) {
					...
					$this->_db->setQuery( $query );
					if (!$this->_db->query()) {
						JError::raiseError( 500, $this->_db->stderr('Delete Data Problem') );
						return false;
					}
